...
lines( grid, dnorm( grid, mean=mu, sd=sigma/sqrt(n)), lwd=3, col='red')
...
lines( grid, (m-1)*dchisq( (m-1)*grid/sigma^2, df=m-1)/sigma^2, lwd=3)
...
for(j in 1:num_sims){
	...
	#x = rnorm(n, mean=mu, sd=sigma)
	x = rcauchy(n, location = mu, scale = sigma)

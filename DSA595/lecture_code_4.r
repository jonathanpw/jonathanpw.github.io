source("lecture_code_4_helper.r")
...
par_index = list( beta=1:p, log_sigma_sq=p+1)

init_par = c( rep( 0, p), log(1))
prior_par = list( prior_mean=rep( 0, p), prior_sd=rep( 5, p))
steps = 20000
burnin = 5000


mcmc_out = mcmc_routine( y, X, init_par, prior_par, par_index, steps, burnin)
...
pdf('trace_plot_adaptive_cov.pdf')
par(mfrow=c(3, 2))
for(r in 1:ncol(chain)){
	...
	abline( v=par_true[r], col='green', lwd=2, lty=2)
}
dev.off()

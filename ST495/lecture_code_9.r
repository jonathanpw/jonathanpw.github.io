...
b0 = runif(p) * 2*sqrt(6/p) + (-sqrt(6/p))
trace = SGD( r=2000, n=n, gradient=df_r, x0=b0, epsilon=10^-3); tail( trace, 2); beta
...
plot( profile[-c(1:120)], main="Trace plot", xlab="iteration", ylab=NA, type="l")
...
for(k in 1:num_sims){
	...
	b0 = runif(p) * 2*sqrt(6/p) + (-sqrt(6/p))
	trace = SGD( r=2000, n=n, gradient=df_r, x0=b0, epsilon=10^-8)
	sampling_dist[k,] = tail( trace, 1)

...
b0 = runif(p) * 2*sqrt(6/p) + (-sqrt(6/p))
trace = SGD( r=100, n=n, gradient=df_r, x0=b0, epsilon=10^-3); 
tail( trace, 2); beta
...
for(k in 1:num_sims){
	...
	b0 = runif(p) * 2*sqrt(6/p) + (-sqrt(6/p))
	trace = SGD( r=30, n=n, gradient=df_r, x0=b0, eta=.001, epsilon=10^-5, max_iter=10^5)
	sampling_dist[k,] = tail( trace, 1)
...
b0 = runif(p) * 2*sqrt(6/p) + (-sqrt(6/p))
trace = MBGD( r=1000, n=n, gradient=df_r, x0=b0); tail( trace, 2); beta
